...
public class CardsServiceImpl implements ICardsService {
  ...
  /**
   * Updates the mobile number of an existing account and sends a notification
   * to update the associated loan mobile number.
   *
   * @param mobileNumberUpdateDto - Data Transfer Object containing the current mobile number and the new mobile number to update
   * @throws ResourceNotFoundException if no account is found with the given current mobile number
   */
  @Override
  @Transactional
  public void updateMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
    ...
    var accounts = cardsRepository.findByMobileNumberAndActiveSw(mobileNumber, true)
        .orElseThrow(
  ...
  public void rollbackMobileNumberUpdate(MobileNumberUpdateDto mobileNumberUpdateDto) {
    ...
    var card = cardsRepository.findByMobileNumberAndActiveSw(mobileNumber, true)
        .orElseThrow(
  ...
  private void updateLoanMobileNumber(MobileNumberUpdateDto mobileNumberUpdateDto) {
    log.info("Sending message to update loan mobile number. Details: {}", mobileNumberUpdateDto);
    var result = streamBridge.send("updateLoanMobileNumber-out-0", mobileNumberUpdateDto);
    if (!result) {
      log.error("Failed to send loan mobile number update message");
      // need to throw an exception here to roll back the local transaction
      // TODO: use custom exception to handle this scenario
      throw new RuntimeException("Failed to send message to loan service");
    }
    log.info("Message sent to update loan mobile number. Result successful: {}", result);
  }
}
